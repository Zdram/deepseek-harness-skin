...
function resolveImagePath(config: Record<string, unknown> = {}): string {
...
(() => {
  ...
  if (!window.__dshDreamObs && document.body) {
    const isSkinNode = (el: Element | null) => {
      if (!el) return false;
      if (el.id === 'deepseek-harness-skin-bg' || el.id === 'deepseek-harness-skin-style') return true;
      const d = el as HTMLElement;
      return !!(d.dataset.dshDreamPanel || d.dataset.dshDreamGlass || d.dataset.composerCard || d.dataset.composerSeat);
    };
    const isComposerNode = (el: Element | null) => {
      ...
      for (const m of muts) {
        if (m.type === 'attributes') {
          ...
          for (const n of [...m.addedNodes, ...m.removedNodes]) {
            ...
            if (isComposerNode(n as Element)) relevant = true;
            else if (isSkinNode(n as Element)) continue;
            else {
